...
import care.data4life.hl7.fhir.r4.test.data.FhirDecimalTestObject
...
class DecimalJsonParserTest : BaseFhirPrimitiveJsonParserTest() {

    @Test
    fun `Given, fromJson() is called with malformed JSON, it throws exception`() {
        assertFails {
            parser.fromJson(Decimal::class, "malformed")
        }
    }
    ...
    @Test
    @Ignore
    fun `Given, fromJson() is called with value and sibling object with extension, it returns a Decimal`() {
        // Given
        val value = 1.0
        val extension = listOf(Extension(url = "some url", valueString = "value as String"))
        val input = FhirDecimalTestObject.jsonData(Decimal(value = value, extension = extension))
        val expected = Decimal(value = value, extension = extension)
    ...
    fun `Given, fromJson() is called with value and sibling object with extension and id, it returns a Decimal`() {
        ...
        val id = "12979787a32339"
        val input =
            FhirDecimalTestObject.jsonData(Decimal(value = value, extension = extension, id = id))
        val expected = Decimal(value = value, extension = extension, id = id)
    ...
    companion object {
        val fhirResourceType = FhirDecimalTestObject::class
    }
}
