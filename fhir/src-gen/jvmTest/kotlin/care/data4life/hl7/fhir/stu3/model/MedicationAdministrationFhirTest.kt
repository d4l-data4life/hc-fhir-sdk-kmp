...
class MedicationAdministrationFhirTest {
    ...
    fun testMedicationAdministration01() {
        // Given
        val sourceJson = loadAsString("stu3/medicationadministrationexample3.json")
    ...
    private fun assertMedicationAdministration01Step01(data: MedicationAdministration) {
        ...
        assertEquals(
            expected = "373147003",
            actual = data.reasonNotGiven?.get(0)?.coding?.get(0)?.code
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "Administration of medication not done due to a contraindication (situation)",
            actual = data.reasonNotGiven?.get(0)?.coding?.get(0)?.display
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "http://snomed.info/sct",
            actual = data.reasonNotGiven?.get(0)?.coding?.get(0)?.system
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = MedicationAdministrationStatus.ON_HOLD,
            actual = data.status
        )
