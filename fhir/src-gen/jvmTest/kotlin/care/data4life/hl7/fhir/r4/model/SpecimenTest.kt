...
class SpecimenTest {
    ...
    fun testSpecimen1() {
        ...
        assertEquals("http://terminology.hl7.org/CodeSystem/v2-0488", data.collection?.method?.coding?.get(0)?.system)
    ...
    fun testSpecimen2() {
        ...
        assertEquals("https://vetmed.iastate.edu/vdl", data.accessionIdentifier?.system)
        assertEquals("20171120-1234", data.accessionIdentifier?.value)
        assertEquals("2017-11-14", data.collection?.collectedDateTime?.value.toString(),)
        assertEquals("James Herriot, FRCVS", data.collection?.collector?.display)
        assertEquals("RTT", data.container?.get(0)?.type?.coding?.get(0)?.code)
        assertEquals("Red Top Tube", data.container?.get(0)?.type?.coding?.get(0)?.display)
        assertEquals("https://vetmed.iastate.edu/vdl", data.container?.get(0)?.type?.coding?.get(0)?.system)
        assertEquals("Red Top Blood Collection Tube", data.container?.get(0)?.type?.text)
        assertEquals("pooled-serum", data.id)
        assertEquals("HTEST", data.meta?.tag?.get(0)?.code)
        assertEquals("test health data", data.meta?.tag?.get(0)?.display)
        assertEquals("http://terminology.hl7.org/CodeSystem/v3-ActReason", data.meta?.tag?.get(0)?.system)
        assertEquals("Pooled serum sample from 30 individuals", data.note?.get(0)?.text)
        assertEquals("Group/herd1", data.subject?.reference)
        assertEquals(NarrativeStatus.GENERATED, data.text?.status)
        assertEquals("Serum sample, pooled", data.type?.coding?.get(0)?.code)
        assertEquals("Serum sample, pooled", data.type?.coding?.get(0)?.display)
        assertEquals("https://vetmed.iastate.edu/vdl", data.type?.coding?.get(0)?.system)
        assertEquals("Pooled serum sample", data.type?.text)
    ...
    fun testSpecimen3() {
        ...
        assertEquals("http://terminology.hl7.org/CodeSystem/v2-0373", data.processing?.get(0)?.procedure?.coding?.get(0)?.system)
        assertEquals("2015-08-18T08:10:00Z", data.processing?.get(0)?.timeDateTime?.value.toString(),)
        assertEquals("2015-08-18T07:03:00Z", data.receivedTime?.value.toString(),)
        assertEquals(SpecimenStatus.AVAILABLE, data.status)
        assertEquals("Patient/example", data.subject?.reference)
        assertEquals(NarrativeStatus.GENERATED, data.text?.status)
        ...
        assertEquals("http://terminology.hl7.org/CodeSystem/v2-0487", data.type?.coding?.get(0)?.system)
    ...
    fun testSpecimen4() {
        ...
        assertEquals("test health data", data.meta?.tag?.get(0)?.display)
        assertEquals("http://terminology.hl7.org/CodeSystem/v3-ActReason", data.meta?.tag?.get(0)?.system)
        assertEquals("ServiceRequest/ft4", data.request?.get(0)?.reference)
    ...
    fun testSpecimen5() {
        ...
        assertEquals("Structure of median cubital vein (body structure)", data.collection?.bodySite?.coding?.get(0)?.display)
        ...
        assertEquals("ServiceRequest/example", data.request?.get(0)?.reference)
        assertEquals(SpecimenStatus.AVAILABLE, data.status)
