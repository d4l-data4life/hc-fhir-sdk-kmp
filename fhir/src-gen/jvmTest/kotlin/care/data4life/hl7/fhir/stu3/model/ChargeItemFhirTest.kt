...
class ChargeItemFhirTest {
    ...
    fun testChargeItem01() {
        // Given
        val sourceJson = loadAsString("stu3/chargeitem-example.json")
    ...
    private fun assertChargeItem01Step01(data: ChargeItem) {
        ...
        assertEquals(
            expected = "224542009",
            actual = data.participant?.get(1)?.role?.coding?.get(0)?.code
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "Coronary Care Nurse",
            actual = data.participant?.get(1)?.role?.coding?.get(0)?.display
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "http://snomed.info/sct",
            actual = data.participant?.get(1)?.role?.coding?.get(0)?.system
                ?.replace("\\n", " ")
        )
        ...
        assertEquals(
            expected = "EUR",
            actual = data.priceOverride?.code
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "urn:iso:std:iso:4217",
            actual = data.priceOverride?.system
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "EUR",
            actual = data.priceOverride?.unit
                ?.replace("\\n", " ")
        )
