...
class NamingSystemFhirTest {
    ...
    fun testNamingSystem01() {
        // Given
        val sourceJson = loadAsString("stu3/namingsystem-example-id.json")
    ...
    fun testNamingSystem02() {
        // Given
        val sourceJson = loadAsString("stu3/namingsystem-example-replaced.json")
    ...
    private fun assertNamingSystem02Step01(data: NamingSystem) {
        ...
        assertEquals(
            expected = "This was a wrong registration for the spanish editions of SNOMED CT. Do not use",
            actual = data.description
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "example-replaced",
            actual = data.id
                ?.replace("\\n", " ")
        )
        ...
        assertEquals(
            expected = "NamingSystem/example",
            actual = data.replacedBy?.reference
                ?.replace("\\n", " ")
        )
        ...
        assertEquals(
            expected = "2.16.840.1.113883.6.96.1",
            actual = data.uniqueId?.get(0)?.value
                ?.replace("\\n", " ")
        )
    }
    ...
    @Test
    fun testNamingSystem03() {
        // Given
        val sourceJson = loadAsString("stu3/namingsystem-example.json")

        // When
        val data = parser.toFhir(NamingSystem::class, sourceJson)

        // Then
        assertNamingSystem03Step01(data)
    ...
    private fun assertNamingSystem03Step01(data: NamingSystem) {
        assertEquals(
