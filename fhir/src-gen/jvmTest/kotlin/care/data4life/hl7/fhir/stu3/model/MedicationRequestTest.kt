...
class MedicationRequestTest {
    ...
    fun testMedicationRequest1() {
        ...
        assertEquals("encounter who leads to this prescription", data.context?.display)
        assertEquals("Encounter/f001", data.context?.reference)
        ...
        assertEquals("TAB", data.dosageInstruction?.get(0)?.doseQuantity?.code)
        assertEquals(
            "http://hl7.org/fhir/v3/orderableDrugForm",
            data.dosageInstruction?.get(0)?.doseQuantity?.system
        )
        assertEquals("TAB", data.dosageInstruction?.get(0)?.doseQuantity?.unit)
        assertEquals("1".toDouble(), data.dosageInstruction?.get(0)?.doseQuantity?.value?.value)
        ...
        assertEquals("Patrick Pump", data.requester?.agent?.display)
        assertEquals("Practitioner/f007", data.requester?.agent?.reference)
        assertEquals("Organization/f002", data.requester?.onBehalfOf?.reference)
        ...
        assertEquals("True".toBoolean(), data.substitution?.allowed?.value)
