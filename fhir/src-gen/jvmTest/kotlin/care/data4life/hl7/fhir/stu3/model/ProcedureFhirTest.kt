...
class ProcedureFhirTest {
    ...
    private fun assertProcedure01Step01(data: Procedure) {
        ...
        assertEquals(
            expected = "310512001",
            actual = data.performer?.get(0)?.role?.coding?.get(0)?.code
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "Medical oncologist",
            actual = data.performer?.get(0)?.role?.coding?.get(0)?.display
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "http://snomed.info/sct",
            actual = data.performer?.get(0)?.role?.coding?.get(0)?.system
                ?.replace("\\n", " ")
        )
    ...
    private fun assertProcedure02Step01(data: Procedure) {
        ...
        assertEquals(
            expected = "398254007",
            actual = data.notDoneReason?.coding?.get(0)?.code
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "  Pre-eclampsia (disorder)",
            actual = data.notDoneReason?.coding?.get(0)?.display
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "http://snomed.info/sct",
            actual = data.notDoneReason?.coding?.get(0)?.system
                ?.replace("\\n", " ")
        )

        assertEquals(
            expected = "Pre-eclampsia",
            actual = data.notDoneReason?.text
                ?.replace("\\n", " ")
        )
        ...
        assertEquals(
            expected = EventStatus.SUSPENDED,
            actual = data.status
        )
    ...
    private fun assertProcedure05Step01(data: Procedure) {
        ...
        assertEquals(
            expected = "Care role",
            actual = data.performer?.get(0)?.role?.text
                ?.replace("\\n", " ")
        )
    ...
    private fun assertProcedure06Step01(data: Procedure) {
        ...
        assertEquals(
            expected = "ProcedureRequest/education",
            actual = data.basedOn?.get(0)?.reference
                ?.replace("\\n", " ")
        )
    ...
    private fun assertProcedure08Step01(data: Procedure) {
        ...
        assertEquals(
            expected = "ProcedureRequest/physical-therapy",
            actual = data.basedOn?.get(0)?.reference
                ?.replace("\\n", " ")
        )
